...
async def async_read_serialnr(hub, address):
    ...
    try:
        ...
        _LOGGER.warning(
            f"{hub.name}: attempt to read serialnumber failed at 0x{address:x}",
            exc_info=True,
        )
    if not res:
        _LOGGER.warning(
    ...
    _LOGGER.info(
        f"Read {hub.name} 0x{address:x} serial number before potential swap: {res}"
    )
    return res
...
class solax_ev_charger_plugin(plugin_base):
    ...
    def matchInverterWithMask(
        ...
        powmatch = ((inverterspec & entitymask & ALL_POW_GROUP) != 0) or (
