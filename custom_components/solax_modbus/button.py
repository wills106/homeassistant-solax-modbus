...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> bool:
    ...
    for button_info in plugin.BUTTON_TYPES:
        if not topology_allows_entity(hub.config, button_info.key):
            _LOGGER.debug(
                "%s: skipping button %s due to topology limits mppt_count=%s battery_count=%s",
                hub.name,
                button_info.key,
...
class SolaXModbusButton(ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        if self._write_method == WRITE_MULTISINGLE_MODBUS:
            ...
            await self._hub.async_write_registers_single(unit=self._modbus_addr, address=self._register, payload=self._command)
