...
CONFIG_SCHEMA = vol.Schema(
    {
        vol.Optional(DOMAIN): vol.Schema(
            {vol.Optional(CONF_DEBUG_SETTINGS): vol.Schema({str: vol.Schema({str: cv.boolean})})},  # Inverter name -> {setting_name: bool}
...
class SolaXModbusHub:
    ...
    def loadLocalData(self):
        try:
            ...
            if self.cyclecount > 5:
                _LOGGER.debug(f"no local data file found after 5 tries - is this a first time run? or didn't you modify any DATA_LOCAL entity?")
    ...
    def treat_address(self, data, regs, idx, descr, initval=0, advance=True):
        ...
        try:
            if descr.unit == REGISTER_U16:
                ...
                val = convert_from_registers(regs[idx : idx + 2], DataType.UINT32, self.plugin.order32)
                words_used = 2
            elif descr.unit == REGISTER_F32:
                val = convert_from_registers(regs[idx : idx + 2], DataType.FLOAT32, self.plugin.order32)
                words_used = 2
            elif descr.unit == REGISTER_S32:
                val = convert_from_registers(regs[idx : idx + 2], DataType.INT32, self.plugin.order32)
                words_used = 2
                ...
                val = (hi + lo * 65536) if self.plugin.order32 == "big" else (lo + hi * 65536)
                words_used = 2
    ...
    async def async_read_modbus_block(self, data, block, typ):
        errmsg = None
        if self.cyclecount < VERBOSE_CYCLES:
            _LOGGER.debug(f"{self._name}: modbus {typ} block start: 0x{block.start:x} end: 0x{block.end:x}  len: {block.end - block.start} regs: {block.regs}")
        try:
            if typ == "input":
    ...
    async def async_read_modbus_registers_all(self, group):
        ...
        for (
            ...
            self.data["_repeatUntil"].items()
        ):  # use a list copy because dict may change during iteration
