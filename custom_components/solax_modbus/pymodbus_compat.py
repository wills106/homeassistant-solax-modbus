...
if _convert_to and _convert_from:
    ...
    if _PM_VER < _v("3.9.2"):  # not fast track, overwrite functions
        ...
        def convert_to_registers(value, dt: DataType, wordorder, string_encoding: str = "utf-8"):  # noqa: F811
        ...
        def convert_from_registers(regs, dt: DataType, wordorder, string_encoding: str = "utf-8"):  # noqa: F811
            global _STARTING
            if _STARTING > 0:
                _STARTING -= 1
    ...
    def convert_from_registers(regs, dt: DataType, wordorder):
        ...
        d = BinaryPayloadDecoder.fromRegisters(list(regs), byteorder=_OldEndian.BIG, wordorder=_old_endian(wordorder))  # all our plugins use this
        if dt == DataType.UINT16:
