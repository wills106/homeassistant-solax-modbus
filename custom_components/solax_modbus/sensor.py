...
def splitInBlocks(descriptions, block_size, auto_block_ignore_readerror):
    ...
    for reg in descriptions:
        ...
        if (not type(descr) is dict) and (
            descr.newblock or ((reg - start) > block_size)
        ):
            if (end - start) > 0:
                _LOGGER.debug(f"Starting new block at 0x{reg:x} ")
                if (
                    (auto_block_ignore_readerror == True)
                    or (auto_block_ignore_readerror == False)
                ) and not descr.newblock:  # automatically created block
                ...
                newblock = block(
                    start=start, end=end, descriptions=descriptions, regs=curblockregs
                )
        ...
        if type(descr) is dict:
            ...
            _LOGGER.debug(
                f"adding register 0x{reg:x} {descr.key} to block with start 0x{start:x}"
            )
            if descr.unit in (
                REGISTER_STR,
                REGISTER_WORDS,
                ...
                if descr.wordcount:
                    ...
                    _LOGGER.warning(
                        f"invalid or missing missing wordcount for {descr.key}"
                    )
            elif descr.unit in (
                REGISTER_S32,
                REGISTER_U32,
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities = []
    groups = {}
    newgrp = lambda: SimpleNamespace(holdingRegs={}, inputRegs={})
    computedRegs = {}
    ...
    for k, v in groups.items():
        ...
        hub_group.holdingBlocks = splitInBlocks(
            holdingRegs, hub.plugin.block_size, hub.plugin.auto_block_ignore_readerror
        )
        hub_group.inputBlocks = splitInBlocks(
            inputRegs, hub.plugin.block_size, hub.plugin.auto_block_ignore_readerror
        )
        hub.computedSensors = computedRegs

        for i in hub_group.holdingBlocks:
            _LOGGER.info(
                f"{hub_name} returning holding block: 0x{i.start:x} 0x{i.end:x} {i.regs}"
            )
        for i in hub_group.inputBlocks:
            _LOGGER.info(
                f"{hub_name} returning input block: 0x{i.start:x} 0x{i.end:x} {i.regs}"
            )
...
def entityToList(
    hub, hub_name, entities, groups, newgrp, computedRegs, device_info, sensor_types
):  # noqa: D103
    for sensor_description in sensor_types:
        if hub.plugin.matchInverterWithMask(
            hub._invertertype,
            sensor_description.allowedtypes,
            hub.seriesnumber,
...
def entityToListSingle(
    hub, hub_name, entities, groups, newgrp, computedRegs, device_info, newdescr
):  # noqa: D103
    ...
    if newdescr.register < 0:  # entity without modbus address
        if newdescr.value_function:
            ...
            _LOGGER.warning(
                f"entity without modbus register address and without value_function found: {newdescr.key}"
            )
    else:
        # target group
        ...
        if newdescr.register_type == REG_HOLDING:
            if (
                newdescr.register in holdingRegs
            ):  # duplicate or 2 bytes in one register ?
                if newdescr.unit in (
                    REGISTER_U8H,
                    REGISTER_U8L,
                    ...
                    holdingRegs[newdescr.register] = {
                        first.unit: first,
                        newdescr.unit: newdescr,
                    }
                else:
                    _LOGGER.warning(
                        f"holding register already used: 0x{newdescr.register:x} {newdescr.key}"
                    )
            else:
            ...
            if newdescr.register in inputRegs:  # duplicate or 2 bytes in one register ?
                ...
                inputRegs[newdescr.register] = {
                    first.unit: first,
                    newdescr.unit: newdescr,
                }
                _LOGGER.warning(
                    f"input register already declared: 0x{newdescr.register:x} {newdescr.key}"
                )
            else:
                inputRegs[newdescr.register] = newdescr
        else:
            _LOGGER.warning(
                f"entity declaration without register_type found: {newdescr.key}"
            )
        ...
        if self.entity_description.key in self._hub.data:
            try:
                val = (
                    ...
                    * self.entity_description.read_scale
                )  # a bit ugly as we might multiply strings or other types with 1
            except:
