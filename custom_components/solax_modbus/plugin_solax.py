...
def value_function_software_version(initval: int, descr: Any, datadict: dict[str, Any]) -> str | None:
    """Unified firmware version display for AC/HYBRID GEN4+ and all MIC types.

    firmware_dsp / firmware_arm always hold the full version ÷100 encoded (158 → 1.58).

    AC/HYBRID — gated on modbus_protocol_version (0x82, only read for AC/HYBRID):
      >= 100 — full values from the combined registers 0x7B (DSP) / 0x7C (ARM)
      <  100 — composed as major × 100 + minor (0x7F/0x7D for DSP, 0x80/0x83 for ARM)
      ARM bootloader suffix: firmware_arm_major (0x80) + bootloader_version (0x84, raw
      minor) — matches SolaX Cloud "ARM Version 1.58-1.15".

    MIC — 0x82 absent (proto is None); registers hold the full ÷100 value directly:
      GEN1 — 0x33D/0x33E   GEN2 — 0x352/0x353 (+ 0x354 boot, ÷100)   GEN4 — 0x394/0x390
      Matches SolaX Cloud "ARM Version 1.36-1.00".
    """
    proto = datadict.get("modbus_protocol_version")
    if proto is None:
        # MIC: full ÷100 values read directly
        dsp_raw = datadict.get("firmware_dsp") or 0
        arm_raw = datadict.get("firmware_arm") or 0
    elif proto >= 100 and datadict.get("firmware_dsp") is not None:
        # AC/HYBRID: combined full-version registers available
        dsp_raw = datadict.get("firmware_dsp") or 0
        arm_raw = datadict.get("firmware_arm") or 0
    else:
        # AC/HYBRID legacy protocol: Full Version = Major × 100 + Minor (per spec)
        dsp_raw = (datadict.get("firmware_dsp_major") or 0) * 100 + (datadict.get("firmware_dsp_minor") or 0)
        arm_raw = (datadict.get("firmware_arm_major") or 0) * 100 + (datadict.get("firmware_arm_minor") or 0)

    dsp_str = f"{dsp_raw // 100}.{dsp_raw % 100:02d}" if dsp_raw else "?.??"
    arm_str = f"{arm_raw // 100}.{arm_raw % 100:02d}" if arm_raw else "?.??"

    if proto is not None:
        # AC/HYBRID: bootloader_version (0x84) is a raw minor; firmware_arm_major (0x80) is its major
        ...
        # MIC GEN2: firmware_arm_boot (0x354) is ÷100 encoded, no separate major register
        arm_boot = datadict.get("firmware_arm_boot")
        if arm_boot is not None:
            arm_str += f"-{arm_boot // 100}.{arm_boot % 100:02d}"
