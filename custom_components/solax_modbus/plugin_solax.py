...
ALLDEFAULT = (
    0  # should be equivalent to AC | HYBRID | GEN2 | GEN3 | GEN4 | GEN5 | X1 | X3
)
...
async def async_read_serialnr(hub, address):
    ...
    try:
        inverter_data = await hub.async_read_holding_registers(
            unit=hub._modbus_addr, address=address, count=7
        )
            ...
            if address == 0x300:
                if res and not res.startswith(("M", "X")):
                    ...
                    ba[0::2], ba[1::2] = (
                        ba[1::2],
                        ba[0::2],
                    )  # swap bytes ourselves - due to bug in Endian.LITTLE ?
                    res = str(ba, "ascii")  # convert back to string
                    hub.seriesnumber = res
            hub.seriesnumber = res
    except Exception as ex:
        _LOGGER.warning(
            f"{hub.name}: attempt to read serialnumber failed at 0x{address:x} data: {inverter_data}",
            exc_info=True,
        )
    if not res:
        _LOGGER.warning(
...
def value_function_remotecontrol_recompute(initval, descr, datadict):
    ...
    if power_control == "Enabled Power Control":
        ...
        power_control == "Enabled Grid Control"
    ):  # alternative computation for Power Control
        if target < 0:
        ...
        power_control == "Enabled Self Use"
    ):  # alternative computation for Power Control
        ap_target = 0 - houseload_nett  # subtract house load
        power_control = "Enabled Power Control"
    elif (
        power_control == "Enabled Battery Control"
    ):  # alternative computation for Power Control
        ap_target = target - pv  # subtract house load and pv
        power_control = "Enabled Power Control"
    elif (
        power_control == "Enabled Feedin Priority"
    ):  # alternative computation for Power Control
        if pv > houseload_nett:
            ap_target = (
                0 - pv + (houseload_brut - houseload_nett) * 1.20
            )  # 0 - pv + (houseload_brut - houseload_nett)
        else:
            ap_target = 0 - houseload_nett
        power_control = "Enabled Power Control"
    elif (
        power_control == "Enabled No Discharge"
    ):  # alternative computation for Power Control
        ...
        ap_target = target
        autorepeat_duration = (
            10  # or zero - stop autorepeat since it makes no sense when disabled
        )
...
def value_function_battery_power_charge(initval, descr, datadict):
    return datadict.get("battery_1_power_charge", 0) + datadict.get(
...
def value_function_software_version_g2(initval, descr, datadict):
    return (
        f"DSP v2.{datadict.get('firmware_dsp')} ARM v2.{datadict.get('firmware_arm')}"
    )


def value_function_software_version_g3(initval, descr, datadict):
    return (
        f"DSP v3.{datadict.get('firmware_dsp')} ARM v3.{datadict.get('firmware_arm')}"
    )


def value_function_software_version_g4(initval, descr, datadict):
    return (
        f"DSP v1.{datadict.get('firmware_dsp')} ARM v1.{datadict.get('firmware_arm')}"
    )
...
def value_function_software_version_air_g3(initval, descr, datadict):
    return (
        f"DSP v2.{datadict.get('firmware_dsp')} ARM v1.{datadict.get('firmware_arm')}"
    )
...
class solax_plugin(plugin_base):
    ...
    async def async_determineInverterType(self, hub, configdict):
        ...
        if not seriesnumber:
            seriesnumber = await async_read_serialnr(
                hub, 0x300
            )  # bug in Endian.LITTLE decoding?
        if not seriesnumber:
        ...
        if seriesnumber.startswith("L30"):
            ...
            self.inverter_model = (
                f"X1-Hybrid-{seriesnumber[1:2]}.{seriesnumber[2:3]}kW SK-TL"
            )
        elif seriesnumber.startswith("U30"):
            invertertype = HYBRID | GEN2 | X1  # Gen2 X1 SK-SU 3kW
            self.inverter_model = (
                f"X1-Hybrid-{seriesnumber[1:2]}.{seriesnumber[2:3]}kW SK-SU"
            )
            ...
            self.inverter_model = (
                f"X1-RetroFit-{seriesnumber[2:3]}.{seriesnumber[3:4]}kW"
            )
            ...
            invertertype = 0
            _LOGGER.error(
                f"unrecognized inverter type - serial number : {seriesnumber}"
            )
    ...
    def matchInverterWithMask(
        ...
        genmatch = ((inverterspec & entitymask & ALL_GEN_GROUP) != 0) or (
            entitymask & ALL_GEN_GROUP == 0
        )
        xmatch = ((inverterspec & entitymask & ALL_X_GROUP) != 0) or (
            entitymask & ALL_X_GROUP == 0
        )
        hybmatch = ((inverterspec & entitymask & ALL_TYPE_GROUP) != 0) or (
            entitymask & ALL_TYPE_GROUP == 0
        )
        epsmatch = ((inverterspec & entitymask & ALL_EPS_GROUP) != 0) or (
            entitymask & ALL_EPS_GROUP == 0
        )
        dcbmatch = ((inverterspec & entitymask & ALL_DCB_GROUP) != 0) or (
            entitymask & ALL_DCB_GROUP == 0
        )
        mpptmatch = ((inverterspec & entitymask & ALL_MPPT_GROUP) != 0) or (
            entitymask & ALL_MPPT_GROUP == 0
        )
        pmmatch = ((inverterspec & entitymask & ALL_PM_GROUP) != 0) or (
            entitymask & ALL_PM_GROUP == 0
        )
        blacklisted = False
        if blacklist:
    ...
    def localDataCallback(self, hub):
        ...
        config_scale_entity = hub.numberEntities.get(
            ...
            if new_max_export != None:
                for key in [
                    ...
                    if number_entity:
                        ...
                        _LOGGER.info(
                            f"local data update callback for entity: {key} new limit: {new_max_export}"
                        )
