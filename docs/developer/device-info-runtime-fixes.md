# Device Info Runtime Bug Fixes & Regression Tests

## Overview

During strict mypy type checking implementation, three critical runtime bugs were discovered and fixed that affected 300+ sensors on system startup. This document details the bugs, fixes, and regression tests added to prevent reintroduction.
...
**File**: `tests/unit/test_device_info_regressions.py`  
**Tests**: 7 new tests  
...
```bash
# Run device info regression tests only
uv run pytest tests/unit/test_device_info_regressions.py -v
